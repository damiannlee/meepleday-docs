...
> **2026-07-26 개정**: 마감임박 알림을 M3에서 빼 [PRD §8.5 장기비전](../prd.md#85-마감임박-알림)으로 이동 — 이 문서는 북마크만 다룸. 알림 재도입 시 아래 §2(구 버전)의 스펙 스케치를 참고해 다시 시작.
> 착수 시 M1·M2 운영 피드백을 반영해 상세 확정 — 현 시점은 스펙 스케치.

## 위치

- 퍼널상 **리텐션 단계** — 발견(M1)·공급(M2)이 선행 ([PRD §3](../prd.md#3-핵심-가치와-퍼널)).
- 로그인 벽은 서비스 전체에서 **여기 한 곳** — 북마크 시점 ([PRD §4.6](../prd.md#46-인증과-로그인-벽)).
- **M3 완료 기준**: 북마크 기능이 정상 동작하고 실사용자가 실제로 북마크를 쌓고 있음. 북극성 지표(주간 재방문)는 알림 도입 전까지 북마크 보유 유저의 재방문율로 잠정 대체 ([PRD §7](../prd.md#7-로드맵-전면-재수립)).

## 범위

1. 북마크 (인증 의존 — 구현 완료)
2. ~~마감임박 이메일 알림~~ — **M3 범위 아님**, [PRD §8.5](../prd.md#85-마감임박-알림) 장기비전으로 이동. 스펙 스케치는 §2에 보존(재도입 시 참고용).
...
## 2. 마감임박 이메일 알림 (보존 — M3 범위 아님)

> **2026-07-26 개정: M3에서 제외, [PRD §8.5](../prd.md#85-마감임박-알림)로 이동.** 아래는 재도입 시 출발점으로 남겨둔 구 스펙 스케치.
...
- **알림을 켜는 시점에 email이 없으면 그 자리에서 입력받음** — 로그인 시점이 아니라 알림 설정 시점. 마찰을 필요한 순간으로 미룸 ([PRD §8.5](../prd.md#85-마감임박-알림)).
- email 없이도 북마크 자체는 동작(목록 관리 용도).

**기각한 대안** ([PRD §8.5](../prd.md#85-마감임박-알림))
...
- 이메일 발송 프로바이더·알림 주기 등 알림 관련 열린 결정은 [PRD §8.5](../prd.md#85-마감임박-알림)로 이동(도입 시점에 확정).
