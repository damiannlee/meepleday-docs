...
> **2026-07-26 개정**: 마감 임박 알림을 M3에서 빼 [PRD §8.5 장기비전](../prd.md#85-마감-임박-알림)으로 이동 — 이 문서는 북마크만 다룸. 알림 재도입 시 아래 §2(구 버전)의 스펙 스케치를 참고해 다시 시작.
...
## 2. 마감 임박 이메일 알림 (보존 — M3 범위 아님)

> **2026-07-26 개정: M3에서 제외, [PRD §8.5](../prd.md#85-마감-임박-알림)로 이동.** 아래는 재도입 시 출발점으로 남겨둔 구 스펙 스케치.

**사용자 스토리**
- 북마크한 이벤트가 마감 임박(ENDING_SOON, `endAt` 48h 이내) 진입 시 이메일 1회 수신.
...
- **알림을 켜는 시점에 email이 없으면 그 자리에서 입력받음** — 로그인 시점이 아니라 알림 설정 시점. 마찰을 필요한 순간으로 미룸 ([PRD §8.5](../prd.md#85-마감-임박-알림)).
- email 없이도 북마크 자체는 동작(목록 관리 용도).

**기각한 대안** ([PRD §8.5](../prd.md#85-마감-임박-알림))
...
- 이메일 발송 프로바이더·알림 주기 등 알림 관련 열린 결정은 [PRD §8.5](../prd.md#85-마감-임박-알림)로 이동(도입 시점에 확정).
